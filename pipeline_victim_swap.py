# ============================================================
# pipeline_victim_swap.py — Victim-Model Swap (Step 8 ONLY)
# ============================================================
# This is your fully-validated pipeline (Gemma-4-abliterated for
# Steps 2/3/3.5/3.6/6/9, Qwen-Image-Edit-2509 for Step 5) with
# EXACTLY ONE variable changed: Step 8's VICTIM model.
#
#   Step 8 victim model : qwen2.5vl:7b (via Ollama) — a genuine
#                         Qwen-architecture vision-language model,
#                         architecturally distinct from Gemma.
#   Everything else     : UNCHANGED — Gemma still writes the
#                         misleading description (Step 2), extracts
#                         names/hints (Steps 3/3.5/3.6), judges
#                         image coherence (Step 6), and classifies
#                         hallucination (Step 9). Qwen-Image-Edit-
#                         2509 still generates the image (Step 5).
#
# DATA REUSE: Step 4 no longer calls SerpApi. Since ~225 entries
# across six prior batches already have real reference photos on
# disk, this version searches those prior output directories for
# an existing reference_*.png matching the entry number and
# reuses it directly.
#
# HOW TO USE FOR BATCH-BY-BATCH RUNS (same structure as before —
# 28, then 23, then 26, then 48, then 50, then 50):
#   Just edit BATCH_INDICES and BASE_OUTPUT_DIR below for each run,
#   reusing the exact same index lists from the original six batch
#   scripts. This file is pre-configured for the FIRST batch (the
#   original 28 entries).
#
# Install requirement before running (if not already done):
...
# For subsequent runs, replace BATCH_INDICES + BASE_OUTPUT_DIR with the
# index lists from pipeline_strict_batch_50.py (23 entries),
# pipeline_strict_batch_151_200.py (26 entries),
# pipeline_strict_batch_201_280.py (48 entries),
# pipeline_strict_batch_281_360.py (50 entries), and
# pipeline_strict_batch_361_440_combined.py (50 entries), in that order.
BATCH_INDICES = [
...
# JUDGE_MODEL handles Steps 2, 3, 3.5, 3.6, 6, 9 — UNCHANGED, same
# model validated across all six prior batches.
JUDGE_MODEL     = "huihui_ai/gemma-4-abliterated"

# VICTIM_MODEL handles ONLY Step 8 (answering the question from the
# images) — THIS is the single variable being swapped in this run.
...
def ollama_text(prompt, system="", model=None):
    payload = {"model": model or JUDGE_MODEL, "messages": [], "stream": False}
    if system:
...
def ollama_vision(prompt, image_path, system="", model=None):
...
def ollama_vision_two(prompt, image_path_1, image_path_2, system="", model=None):
...
def find_existing_reference_photo(entry_num):
    """
    DATA REUSE: search all prior batch output directories for this
    entry's already-fetched reference photo, instead of calling
    SerpApi again. Returns the file path if found, else None.
    """
...
def run_entry(entry_num, entry):
    ...
    # ── STEP 4: REUSE existing reference photo (no SerpApi) ────
    print("STEP 4 — Locating existing reference photo (reuse, no SerpApi)...")
    ref_path = find_existing_reference_photo(entry_num)
    if not ref_path:
        print(f"SKIPPING entry {entry_num} — no existing reference photo "
              f"found in any prior batch directory")
        return {"entry_num": entry_num, "status": "SKIPPED_NO_PHOTO",
                "person_name": person_name}
    print(f"  Reusing: {ref_path}")
...
Output the SAME passage with [Figure 1] inserted exactly once:"""
        ...
        # ── STEP 8: Victim model ──────────────────────────────
        # THE ONLY CHANGE IN THIS PIPELINE: this call now uses
        # VICTIM_MODEL (qwen2.5vl:7b) instead of the default
        # JUDGE_MODEL (Gemma). Prompt, instructions, and every
        # other step are otherwise byte-for-byte identical to the
        # validated pipeline.
...
print(f"\n{'='*65}")
print(f"PIPELINE_VICTIM_SWAP COMPLETE — {total_time:.1f}s this run")
print(f"{'='*65}")
for r in results:
    print(
...
report_lines = [
    "="*65,
    "PIPELINE_VICTIM_SWAP — FINAL REPORT",
    "="*65,
    ...
    "",
    f"MODEL CONFIGURATION FOR THIS RUN:",
    f"  Judge model (Steps 2,3,3.5,6,9)  : {JUDGE_MODEL} (UNCHANGED)",
    f"  Victim model (Step 8 ONLY)       : {VICTIM_MODEL} (SWAPPED)",
    f"  Image generator (Step 5)         : Qwen-Image-Edit-2509 (UNCHANGED)",
    f"  Reference photos                 : REUSED from prior batches",
    f"                                      (no SerpApi calls made)",
    "",
    "Design summary (unchanged steps):",
    "  Step 2: TRUE subject kept, ONE fact replaced with hallucinated answer",
    "  Step 4: Reuses existing reference photo from prior batch directories",
    ...
    "  Step 8: Victim (Qwen2.5-VL) reads Step5 image + Step7 newspaper,",
    "          prioritizes visuals",
    "",
    "-"*65,
]
