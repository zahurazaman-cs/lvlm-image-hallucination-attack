# ============================================================
# pipeline_reclassify_all_qwen25_hallucination_analysis.py
# ============================================================
# Tests whether using the professor-approved HALLUCINATION_ANALYSIS
# Step 8 prompt (RISK framing tags before the ORIGINAL unchanged
# instructions, a MANDATORY FINAL CHECK and FINAL_RULE after them,
# the whole thing wrapped in matching opening and closing tags)
...
#            image(s), with the SAME professor-approved
#            HALLUCINATION_ANALYSIS prompt validated in the Gemma
#            reclassification test and in the 107-entry Qwen2.5-VL
#            defense experiment. ONE pass per entry — no
#            escalation loop.
#   Step 9 (UNCHANGED) — Gemma-4-abliterated classifies the new
#            answer natively, same judge prompt as always.
#
# A single fixed version of the prompt is used for every entry.
#
...
OUTPUT_DIR = "outputs_step8_reclassify_all_qwen25_hallucination_analysis"
STATE_PATH = f"{OUTPUT_DIR}/entry_state.json"

# Professor-approved defense prompt, single fixed version, wraps
# the entire original instruction block inside one
# HALLUCINATION_ANALYSIS tagged section. Identical wording to the
# Gemma all-500 hallucination-analysis test. Since this is a single
# pass per entry, there is no history of prior wrong answers.
...
def wrapped_prompt(question):
    ...
    Qwen2.5-VL's original victim prompt), wrapped inside the
    professor-approved HALLUCINATION_ANALYSIS block. Every
    reference to the photograph or the newspaper outside the
    original block is written explicitly as Image 1 or Image 2."""
...
print(f"\n{'='*65}")
print(f"RECLASSIFY ALL QWEN2.5-VL — HALLUCINATION_ANALYSIS prompt (Step 8), Step 9 unchanged")
...
print(f"\n{'='*65}")
print("RECLASSIFY ALL QWEN2.5-VL — FINAL SUMMARY (HALLUCINATION_ANALYSIS Step 8 prompt)")
print(f"{'='*65}")
