# ============================================================
# make_comparison_three_defense_conditions.py
# ============================================================
# THREE-WAY comparison chart for the results section of the
# paper, showing the effect of the two prompt-level defenses
# across the full 500-entry dataset, with clear annotation of
# dataset identity, victim model, and total entries so the chart
# is self-explanatory to a reviewer with no other context.
#
#   Condition 1: No Defense (the original attack, no prompt-level
#     mitigation applied — the baseline)
...
#   Condition 3: Structured Hallucination-Analysis Defense (a tagged
#     HALLUCINATION_ANALYSIS block wrapping the SAME unchanged
#     original instructions, including explicit risk framing,
#     a mandatory verification check, and a final override rule)
#
# All three conditions use the SAME 500 entries, the SAME
# adversarial images (Steps 1-7 never regenerated across any
# condition), the SAME victim model (Gemma-4-abliterated), and the
# SAME judge model (Gemma-4-abliterated, unchanged throughout).
# The ONLY variable across the three conditions is the wording of
# the Step 8 instruction shown to the victim model.
#
...
#
# Saves the chart to:
#   outputs_step8_reclassify_all500_hallucination_analysis/comparison_three_defense_conditions.png
#
...
COND1_TARGETED   = 229
COND1_UNTARGETED = 103
COND1_NONE       = 168

# ── Condition 2: Minimal Prompt Defense (already known) ─────────
# 281/500 total hallucination (56.2%). Targeted/untargeted split
# for this condition was not separately reported at the time, so
# it is shown here as a single combined hallucination wedge unless
# you have the split saved — edit COND2_TARGETED/COND2_UNTARGETED
# below if you have those two numbers on hand.
COND2_HALLUCINATED = 281
COND2_NONE         = TOTAL_ENTRIES - COND2_HALLUCINATED

# ── Condition 3: Structured Hallucination-Analysis Defense (read live) ───
COND3_STATE_PATH = "outputs_step8_reclassify_all500_hallucination_analysis/entry_state.json"
...
CHART_PATH = ("outputs_step8_reclassify_all500_hallucination_analysis/"
              "comparison_three_defense_conditions.png")

# ── Print summary to terminal ────────────────────────────────────
cond1_total = COND1_TARGETED + COND1_UNTARGETED + COND1_NONE
cond1_hall_pct = 100 * (COND1_TARGETED + COND1_UNTARGETED) / cond1_total
cond2_hall_pct = 100 * COND2_HALLUCINATED / TOTAL_ENTRIES
...
print(f"Victim model: Gemma-4-abliterated")
print(f"Total entries: {TOTAL_ENTRIES}")
print()
print(f"Condition 1, No Defense                    : "
      f"{COND1_TARGETED + COND1_UNTARGETED}/{cond1_total} "
      f"({cond1_hall_pct:.1f}%) hallucinated")
print(f"Condition 2, Minimal Prompt Defense         : "
      f"{COND2_HALLUCINATED}/{TOTAL_ENTRIES} "
...
# Panel 1: No Defense (3-way split available)
ax = axes[0]
sizes = [COND1_TARGETED, COND1_UNTARGETED, COND1_NONE]
labels = [
    f"Targeted\n({COND1_TARGETED}, {100*COND1_TARGETED/cond1_total:.1f}%)",
    f"Untargeted\n({COND1_UNTARGETED}, {100*COND1_UNTARGETED/cond1_total:.1f}%)",
    f"No Hallucination\n({COND1_NONE}, {100*COND1_NONE/cond1_total:.1f}%)",
]
...
# Panel 2: Minimal Prompt Defense (2-way, combined hallucination)
ax = axes[1]
sizes = [COND2_HALLUCINATED, COND2_NONE]
labels = [
    f"Hallucinated\n({COND2_HALLUCINATED}, {cond2_hall_pct:.1f}%)",
    f"No Hallucination\n({COND2_NONE}, {100-cond2_hall_pct:.1f}%)",
]
wedges, texts, autotexts = ax.pie(
...
fig.suptitle(
    ...
    f"Gemma-4-abliterated  |  Judge Model: Gemma-4-abliterated  |  "
