# ============================================================
# combine_piechart_till_200_qwen25.py
# ============================================================
# Combines results from ALL THREE Qwen2.5-VL victim-model batches
# completed so far, covering the same entries as the first three
# Gemma-victim batches (lines 1-200 of qa_data.json):
...
#     NEW reuse method — Steps 1-7 images/newspapers reused
#     directly from the Gemma-victim run, only Steps 8/9 rerun)
#
# Victim model (target model, Step 8): Qwen2.5-VL 7B (via Ollama)
# Judge model (Step 9 classifier): Gemma-4-abliterated (UNCHANGED
# across all three batches — only the victim model is the
# experimental variable)
# Image generator: Qwen-Image-Edit-2509 (UNCHANGED; batch 3's
# images were generated during the ORIGINAL Gemma-victim run and
# reused as-is, not regenerated)
#
# Produces ONE combined pie chart across all 77 entries showing:
#   - TARGETED hallucination
#   - UNTARGETED hallucination
#   - NONE (no hallucination)
#
# Saves the chart to:
#   outputs_victim_qwen_151_200/combined_piechart_till_200_qwen25.png
#
...
CHART_PATH    = "outputs_victim_qwen_151_200/combined_piechart_till_200_qwen25.png"
...
if scored_total == 0:
    ...
    ax.set_title(
        ...
        f"(28 + 23 + 26 entries across three batches, {skipped} skipped)\n"
