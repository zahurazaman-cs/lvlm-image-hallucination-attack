# ============================================================
# ever_hallucinated_report_till_200_qwen25.py
# ============================================================
# Combines all THREE Qwen2.5-VL victim-model batches:
#   - outputs_victim_qwen_28        (28 entries, two-phase pipeline
#     — has MULTIPLE iterations per entry via step9_hallucination_
#     iterN.txt files)
#   - outputs_victim_qwen_50        (23 entries, two-phase pipeline
#     — same multi-iteration structure)
#   - outputs_victim_qwen_151_200   (26 entries, NEW reuse method —
#     only ONE pass per entry, step9_hallucination.txt, since the
#     final Gemma-escalated image was reused directly rather than
#     re-escalated)
#
...
# Gemma-victim reports. For the reuse-method batch, there is only
# one iteration per entry by design, so "final" and "ever" are
# identical for those entries — this is noted explicitly rather
# than treated as a coincidence.
#
# Writes: Ever_hallucinated_report_till_200_qwen25.txt
# ============================================================
...
REUSE_BATCH_DIR = "outputs_victim_qwen_151_200"
REPORT_PATH = "Ever_hallucinated_report_till_200_qwen25.txt"
...
# ── Reuse-method batch: single pass per entry ───────────────────
with open(f"{REUSE_BATCH_DIR}/entry_state.json", "r") as f:
    reuse_state = json.load(f)
for key, s in reuse_state.items():
    if s["status"] in ("SKIPPED_NO_PHOTO", "ERROR"):
        continue
    all_entries[(REUSE_BATCH_DIR, s["entry_num"])] = [s["status"]]
...
lines.append("Combined across outputs_victim_qwen_28 + "
              "outputs_victim_qwen_50 + outputs_victim_qwen_151_200")
lines.append("")
lines.append("NOTE: outputs_victim_qwen_151_200 used the 'reuse' method —")
lines.append("Steps 1-7 images/newspapers were taken directly from the")
lines.append("Gemma-victim run's FINAL iteration and reused as-is; only")
lines.append("Steps 8/9 were rerun with Qwen2.5-VL as victim. These")
lines.append("entries therefore have exactly ONE pass each, so their")
lines.append("'final' and 'ever hallucinated' values are identical by")
lines.append("construction, not because escalation happened to stop early.")
lines.append("=" * 65)
