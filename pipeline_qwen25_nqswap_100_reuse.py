# ============================================================
# pipeline_qwen25_nqswap_100_reuse.py
# ============================================================
# Same reuse method validated on qa_data.json: reuses Steps 1-7
# already completed by the Gemma-victim NQ-Swap run (misleading
# description, adversarial image, coherence check, newspaper PNG)
# — NOTHING is regenerated. Only re-runs, across ALL 100 NQ-Swap
# entries in one go:
#
...
#
# Pulls entries from all FOUR completed NQ-Swap Gemma batches:
...
#
# right_answer/hallucinated_answer/iterations_needed come from each
# batch's own SUMMARY.json regardless of which classification
# scheme that batch originally used.
#
...
OUTPUT_DIR    = "outputs_victim_qwen_nqswap_100"
...
print(f"\n{'='*65}")
print(f"NQ-SWAP 100 — STEP 8+9 REUSE — Victim: {VICTIM_MODEL}  "
      f"|  Judge: {JUDGE_MODEL}")
...
for e in all_entries:
    ...
    print(f"\n{'='*65}")
    print(f"{key}  (reusing iteration {final_iter} from source)")
    print(f"{'='*65}")
...
Do NOT say "not stated" or "unknown":"""
    ...
    with open(f"{out_entry_dir}/step8_victim_answer.txt", "w") as f:
        f.write(
            ...
            f"(Reused from {e['batch_dir']}/entry_{e['entry_num']}, "
...
print(f"\n{'='*65}")
print(f"COMPLETE — NQ-Swap 100 entries -> {OUTPUT_DIR}")
print(f"{'='*65}")
