# ============================================================
# pipeline_strict_batch_201_280.py — Strict Pipeline, Lines 201-280
# ============================================================
# Scans lines 201-280 of qa_data.json (the next 80 lines, requested
# specifically to find 50 qualifying entries) for human-celebrity
# entries — same criteria as before: a real, identifiable,
# non-fictional person (or a simple two-person real comparison),
# excluding bands/groups of 3+, fictional characters, companies,
# places, and abstract facts. 48 qualifying entries were found
# (this stretch had many place-name/plant-taxonomy/abstract-fact
# entries, so 48 was the natural yield rather than a full 50).
#
...
#
# Output directory: outputs_strict_batch_201_280
# ============================================================
...
# 0-indexed line numbers in qa_data.json for the 48 human-celebrity
# entries identified within lines 201-280.
#
#   line#(1-idx) -> subject
#   201 -> Don Henley / Glenn Frey (Eagles musicians)
#   203 -> Kamal Shalorus / Matthew Granahan (MMA fighters)
#   204 -> Nick Love (film director)
#   205 -> Otto Stern / Max Volmer (Nobel-winning scientists)
#   206 -> Neil LaBute / H. Bruce Humberstone (film directors)
#   208 -> Lee Ranaldo / Mary Ramsey (musicians)
#   210 -> Jocelyn Bell Burnell / Antony Hewish (astrophysicists)
#   211 -> Britney Spears
#   212 -> Rihanna
#   213 -> Mats Hummels / Jonas Hummels (footballers)
#   214 -> Fabian Cowdrey / Colin Cowdrey (cricketers)
#   215 -> Judy Woodruff (journalist/anchor)
#   216 -> Christy Canyon / Jack Kevorkian
#   217 -> Bill Clinton / Peter F. Paul
#   223 -> Ming-Na Wen (actress)
#   225 -> Prince Andrew / Queen Elizabeth II
#   228 -> Kōbō Abe / Agatha Christie (authors)
#   233 -> Howard Ashman / Alan Menken (composers)
#   234 -> Ken Leung (actor)
#   235 -> J.H. Williams III (comic artist)
#   236 -> Nick Kroll (actor/comedian)
#   237 -> Rudy Giuliani
#   238 -> Thomas Harris (author)
#   240 -> John Bruce Yeh (clarinetist)
#   241 -> Mark L. Lester / Ken Loach (directors)
#   245 -> Anthony Hopkins / Jessica Lange (actors)
#   248 -> Xavier Malisse / Virginia Wade (tennis players)
#   249 -> Thom Yorke (Radiohead)
#   250 -> Aaron Galindo (footballer)
#   251 -> Patti Smith (musician/poet)
#   252 -> Lance Stephenson (NBA player)
#   253 -> Maria Brontë / Patrick Brontë
#   254 -> Henry Bacon (architect)
#   255 -> Lawrence Tanenbaum (businessman)
#   256 -> Allan Dwan / George Marshall (directors)
#   257 -> Johnny Carson
#   258 -> Carlos Bacca / Cristiano Ronaldo (footballers)
#   262 -> Anne Frank / John Kennedy Toole
#   263 -> Richard Wagner / Christoph Gluck (composers)
#   264 -> Nicholas Sposato / Rahm Emanuel (politicians)
#   265 -> Jeff Green (NBA player)
#   267 -> Lyudmila Petrushevskaya (author)
#   268 -> Maurice Newman (businessman)
#   270 -> Aadesh Shrivastava (music director)
#   273 -> David Gordon Green / Larry Hagman (filmmaker/actor)
#   274 -> Jack Baldwin (racing driver)
#   276 -> Hugh Grant (actor)
#   277 -> Frank Darabont (director)
BATCH_INDICES = [
    200, 202, 203, 204, 205, 207, 209, 210, 211, 212, 213, 214,
    215, 216, 222, 224, 227, 232, 233, 234, 235, 236, 237, 239,
    240, 244, 247, 248, 249, 250, 251, 252, 253, 254, 255, 256,
    257, 261, 262, 263, 264, 266, 267, 269, 272, 273, 275, 276
]

BASE_OUTPUT_DIR = "outputs_strict_batch_201_280"
...
print(f"\n{'='*65}")
print(f"PIPELINE_STRICT_BATCH_201_280 COMPLETE — {total_time:.1f}s this run")
print(f"{'='*65}")
for r in results:
    print(
...
report_lines = [
    "="*65,
    "PIPELINE_STRICT_BATCH_201_280 — FINAL REPORT",
    "="*65,
