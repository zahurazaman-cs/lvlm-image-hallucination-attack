# ============================================================
# make_iterations_to_any_hallucination_report_qwen25.py
# ============================================================
# Analyzes iteration data for hallucination (TARGETED or
# UNTARGETED) across ALL FOURTEEN Qwen2.5-VL victim-model batches.
#
# IMPORTANT METHODOLOGICAL NOTE — read before interpreting results:
#
#   TWO batches (outputs_victim_qwen_28, outputs_victim_qwen_50,
#   51 entries) used the TWO-PHASE pipeline, which genuinely
#   escalated FRESH images specifically to try to fool Qwen2.5-VL,
#   up to 5 attempts each. For these, "iteration of first
#   hallucination" means exactly what it meant in the Gemma
#   analysis: how many escalation attempts it took.
#
#   The other TWELVE batches (450 entries) used the REUSE method —
#   they never generated or escalated anything for Qwen2.5-VL.
#   They simply took whichever image Gemma's own attack pipeline
#   had already settled on (recorded as "source_iteration" in each
#   entry's state) and tested Qwen2.5-VL against that EXACT SAME
#   image, once. So for these entries, the iteration number does
#   NOT mean "how many attempts it took to fool Qwen2.5-VL" — it
#   means "which escalation level of GEMMA's attack image also
#   happened to transfer and fool Qwen2.5-VL." This answers a
#   related but different question: do harder-to-fool-Gemma images
#   also transfer better/worse to a different victim model?
#
# This script reports BOTH groups separately AND combined, with
# clear labeling, so the distinction is never silently conflated.
#
# Saves the chart to:
#   iterations_to_any_hallucination_report_qwen25.png
#
...
CHART_PATH = "iterations_to_any_hallucination_report_qwen25.png"
...
fig.suptitle(
    "Qwen2.5-VL Victim Pipeline — Iteration Analysis for Hallucination\n"
    "(Targeted + Untargeted combined; two methodologically distinct "
