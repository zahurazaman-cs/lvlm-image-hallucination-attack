# ============================================================
# pipeline_strict_batch_nqswap_76_100.py — NQ-Swap Real-World
# Dataset, Fourth Batch of 25 (Entries 76-100)
# ============================================================
# Same 10-step design used across all qa_data.json (HaluEval)
# batches, now run on entries 75-99 of the NQ-Swap real-world
# generalization dataset (pminervini/NQ-Swap on HuggingFace,
# "Entity-Based Knowledge Conflicts in Question Answering",
# Longpre et al.) — NOT derived from HotpotQA/HaluEval at all.
#
# Uses the CORRECT native 3-way TARGETED/UNTARGETED/NONE
# classification scheme (this template is copied from batch_50,
# NOT batch_28 — entries 0-49 in this same dataset used the older
# binary scheme by mistake; this brings the dataset to 100 total entries, the full
# generalization-test target size discussed earlier).
#
import os
import json
import base64
...
# Entries 50-74 (0-indexed) in qa_data_nqswap.json — the THIRD
# batch of 25 from NQ-Swap (pminervini/NQ-Swap), pre-filtered for
# named, real, photographable individuals on both the true and
# hallucinated sides. This is a genuinely independent real-world
# dataset, not derived from HotpotQA/HaluEval like qa_data.json.
#
# NOTE: unlike entries 0-49 (which used the OLDER binary
# HALLUCINATED/DID_NOT_HALLUCINATE scheme inherited from an early
# template), this batch uses the CORRECT native 3-way
# TARGETED/UNTARGETED/NONE scheme, matching every other batch in
# the main qa_data.json pipeline.
BATCH_INDICES = list(range(75, 100))

BASE_OUTPUT_DIR = "outputs_strict_batch_nqswap_76_100"
...
print(f"\n{'='*65}")
print(f"PIPELINE_STRICT_BATCH_NQSWAP_76_100 COMPLETE — {total_time:.1f}s this run")
print(f"{'='*65}")
for r in results:
    print(
...
report_lines = [
    "="*65,
    "PIPELINE_STRICT_BATCH_NQSWAP_76_100 — FINAL REPORT",
    "="*65,
