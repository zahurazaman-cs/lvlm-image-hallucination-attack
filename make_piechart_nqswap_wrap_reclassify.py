# ============================================================
# make_piechart_nqswap_wrap_reclassify.py
# ============================================================
# Pie chart for the NQ-Swap BALANCED WRAP-AROUND DEFENSE
# reclassification (outputs_nqswap_100_wrap_reclassify) — the
# ORIGINAL Step 8 attack instructions were left completely
# UNCHANGED, with a short CAUTION added immediately before them
# and a short REMINDER immediately after. Victim + judge: both
# Gemma-4-abliterated. Reuses all 100 existing NQ-Swap images
# (Steps 1-7 untouched).
#
# Shows the NEW TARGETED / UNTARGETED / NONE breakdown after the
# defense wrap was applied, directly comparable to the original
# (undefended) NQ-Swap pie chart.
#
# Saves the chart to:
#   outputs_nqswap_100_wrap_reclassify/piechart_nqswap_wrap_reclassify.png
#
...
STATE_PATH = "outputs_nqswap_100_wrap_reclassify/entry_state.json"
CHART_PATH = ("outputs_nqswap_100_wrap_reclassify/"
              "piechart_nqswap_wrap_reclassify.png")
...
print()
print(f"Entries that flipped from hallucinating -> correct: {changed_to_correct}")
print(f"Entries that flipped from correct -> hallucinating: {changed_to_wrong}")
...
ax.set_title(
    "NQ-Swap — Defense Wrap-Around Reclassification\n"
    f"Total Hallucination Rate: {total_hall_pct:.1f}% "
    f"(Targeted + Untargeted, {scored_total} entries)\n"
    "Victim + Judge: Gemma-4-abliterated  |  "
    "CAUTION/REMINDER wrap around unchanged Step 8 instructions",
