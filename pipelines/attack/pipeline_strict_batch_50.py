# ============================================================
# pipeline_strict_batch_50.py — Strict Pipeline, Next 50 Lines
# ============================================================
# Scans lines 101-150 of qa_data.json (the next 50 after the first
# 100 already covered) for human-celebrity entries — same criteria
# as before: a real, identifiable, non-fictional person (or a
# simple two-person real comparison), excluding bands/groups of
# 3+, fictional characters, companies, places, and abstract facts.
# 23 qualifying entries were found (see BATCH_INDICES below).
#
# Carries over BOTH fixes validated in earlier runs:
...
#   2. Step 9 hallucination check is NATIVELY 3-WAY (built in,
#      not a post-hoc reclassification pass this time):
#        TARGETED   — victim's answer matches/implies the specific
#                      hallucinated_answer (the misleading image
#                      worked as intended)
...
#   Step 10 only stops early on TARGETED (the definitive success
#   condition); UNTARGETED and NONE both continue escalating up to
#   MAX_STEP10_ITER, since only TARGETED confirms the image caused
#   the SPECIFIC intended false belief.
#
# Output directory: outputs_strict_batch_50
# ============================================================
...
# 0-indexed line numbers in qa_data.json for the 23 human-celebrity
# entries identified within lines 101-150 (the next 50 after the
# first 100 already covered).
#
#   line#(1-idx) -> subject
#   101 -> Prince William, Duke of Gloucester / Prince George of Denmark
#   105 -> John Beilein (basketball coach)
#   106 -> Chris Noonan (film director)
#   108 -> Yeon Woo-jin (actor)
#   110 -> Cheryl Campbell (actress)
#   111 -> Bob Barker (TV host)
#   113 -> Robert Glenister (actor)
#   116 -> Albert Pierrepoint (historical executioner)
#   120 -> Erwin Rommel (WWII general)
#   124 -> Gary Harrison / Bryan White (musicians)
#   125 -> James Hunt (F1 driver)
#   127 -> Anna Kournikova (tennis player)
#   130 -> Craig Newmark (Craigslist founder)
#   131 -> Kim Clijsters / Mary Pierce (tennis players)
#   134 -> Jo Ann Terry (Olympic athlete)
#   135 -> Pearl Lowe / Alison Goldfrapp (musicians)
#   137 -> Emrich "Imi" Lichtenfeld (Krav Maga founder)
#   138 -> Robert Fleischman / Jimmy Barnes (musicians)
#   140 -> James Chaney (civil rights figure)
#   141 -> Conrad Black (media tycoon)
#   145 -> Edwyn Collins (musician)
#   148 -> Park Dietz (forensic psychiatrist)
#   149 -> Roy Orbison (musician)
BATCH_INDICES = [
    100, 104, 105, 107, 109, 110, 112, 115, 119, 123, 124, 126,
    129, 130, 133, 134, 136, 137, 139, 140, 144, 147, 148
]

BASE_OUTPUT_DIR = "outputs_strict_batch_50"
...
print(f"\n{'='*65}")
print(f"PIPELINE_STRICT_BATCH_50 COMPLETE — {total_time:.1f}s this run")
print(f"{'='*65}")
for r in results:
    print(
...
report_lines = [
    "="*65,
    "PIPELINE_STRICT_BATCH_50 — FINAL REPORT",
    "="*65,
