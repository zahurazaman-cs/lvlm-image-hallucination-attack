# ============================================================
# pipeline_strict_batch.py — Stricter Test (2 entries)
# ============================================================
# Same 10-step design as pipeline_correct_batch.py, with an
# ADDITIONAL constraint on Step 5:
#
#   pipeline_correct_batch.py fixed the "literal claim sentence"
#   leak (e.g. "Henri Leconte: Pioneer of Grand Slam Victory (15
#   Titles)"), but still allowed the WRONG PERSON'S NAME to appear
#   as a label (e.g. a plaque/pedestal reading "Henri Leconte" next
#   to a "15"). That's still close to a free win — the victim just
#   has to read a name that happens to match the hallucinated
#   answer, with no genuine visual inference required.
#
#   STRICTER FIX: the visual element must now contain NO PERSON
#   NAMES at all — not the real person's name, not the wrong
#   person's name, not any name. Only numbers, symbols, flags,
#   objects, dates, or generic labels. The victim model must
#   connect the visual claim (a number, a flag, an object) to the
#   PERSON'S IDENTITY purely through:
#     (a) the person's face/likeness in the photo (Step 4/5), and
#     (b) the article/context text in the newspaper (Step 7)
#   This is a genuinely harder, more visually-inferential task and
#   a much better test of "does a misleading image cause
#   hallucination" rather than "can a VLM read text off a picture."
#
# Test entries (same as pipeline_correct_batch.py):
#   Entry #4 (index 3) — Margaret "Peggy" Seeger
#   Entry #6 (index 5) — Jonathan Stark
#
# Output directory: outputs_strict_batch
# ============================================================
...
BASE_OUTPUT_DIR = "outputs_strict_batch"
...
print(f"\n{'='*65}")
print(f"PIPELINE_STRICT_BATCH COMPLETE — {total_time:.1f}s this run")
print(f"{'='*65}")
for r in results:
    print(
...
report_lines = [
    "="*65,
    "PIPELINE_STRICT_BATCH — FINAL REPORT",
    "="*65,
