# ============================================================
# pipeline_strict_batch_nqswap_25.py — NQ-Swap Real-World Dataset,
# 20 Entries
# ============================================================
# Same 10-step design and Step 5 name-free constraint validated
# across all 500 qa_data.json (HaluEval) entries, now run on a
# GENUINELY DIFFERENT real-world dataset: NQ-Swap
# (pminervini/NQ-Swap on HuggingFace), sourced from "Entity-Based
# Knowledge Conflicts in Question Answering" (Longpre et al.),
# built on Natural Questions — NOT derived from HotpotQA/HaluEval
# at all. This tests whether the pipeline's findings generalize
# beyond the original benchmark.
#
# All 20 entries in qa_data_nqswap.json (25 entries) were manually selected from
# NQ-Swap's dev split specifically because BOTH the true answer
# (org_answer) and the substituted/false answer (sub_answer) are
# real, named, photographable individuals — matching this
# pipeline's requirement for a person-based visual attack.
#
#   STRICT FIX (carried over): the visual element and any PIL
#   overlay text must contain NO PERSON NAMES at all — not the real
#   person's name, not the wrong-answer person's name. Only numbers,
#   dates, generic words, flags, or objects are allowed. The victim
#   model must connect the visual clue to the person's identity via
#   the photo's likeness + article context alone — not by reading a
#   name off the image.
#
# Output directory: outputs_strict_batch_28
# ============================================================
...
# All 20 entries in qa_data_nqswap.json are already pre-filtered
# for named, real, photographable individuals (both the true and
# substituted/hallucinated answers), sourced from NQ-Swap
# (pminervini/NQ-Swap on HuggingFace — "Entity-Based Knowledge
# Conflicts in Question Answering", Longpre et al.). This is a
# genuinely independent real-world dataset, not derived from
# HotpotQA/HaluEval like qa_data.json.
BATCH_INDICES = list(range(25, 50))

BASE_OUTPUT_DIR = "outputs_strict_batch_nqswap_26_50"
...
print(f"\n{'='*65}")
print(f"PIPELINE_STRICT_BATCH_NQSWAP_26_50 COMPLETE — {total_time:.1f}s this run")
print(f"{'='*65}")
for r in results:
    print(
...
report_lines = [
    "="*65,
    "PIPELINE_STRICT_BATCH_NQSWAP_26_50 — FINAL REPORT",
    "="*65,
    ...
    "",
    "KEY FIX vs pipeline_correct_batch.py:",
    "  Step 5 visual element + PIL overlay text now contains NO",
    "  person names at all (real or wrong-answer name). Only",
    "  numbers, dates, generic words, flags, or objects are allowed.",
    "  The victim model must connect the visual clue to the person's",
    "  identity via the photo's likeness + article context alone —",
    "  not by reading a name off the image.",
    "",
