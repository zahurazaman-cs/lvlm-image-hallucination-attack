# ============================================================
# pipeline_strict_batch_361_440_combined.py — Strict Pipeline
# ============================================================
# This batch combines TWO sources of entries:
#
#   1. The 8 entries dropped from the 281-360 batch to stay within
#      SerpApi quota: Julie Cooper, Trevor Prangley, Prince Hall,
#      Betzy Kjelsberg, Byron Edmund Walker, Andrés D'Alessandro,
#      Keith Park/Arthur Noss, and Dirkie Chamberlain.
#
#   2. 42 new human-celebrity entries identified within lines
#      361-440 of qa_data.json (same criteria as before: a real,
#      identifiable, non-fictional person or simple two-person
#      real comparison, excluding bands/groups of 3+, fictional
#      characters, companies, places, abstract facts). This
#      stretch had an unusually low yield (39 on first pass) due
#      to heavy place-name/company/group-trivia density, so 3
#      borderline-but-legitimate entries were included to reach 42
#      (John the Blind via the Psalter entry, Willie Nelson via
#      the Highwaymen entry, and LBJ via the Thomas Mann entry).
#
# Total: 50 entries.
#
...
#
# Output directory: outputs_strict_batch_361_440_combined
# ============================================================
...
# 0-indexed line numbers in qa_data.json — 8 previously-dropped
# entries from the 281-360 range + 42 new entries from 361-440.
#
#   --- Previously dropped (from 281-360) ---
#   290 -> Julie Cooper (politician)
#   316 -> Trevor Prangley (MMA fighter)
#   332 -> Prince Hall
#   333 -> Betzy Kjelsberg (politician)
#   337 -> Byron Edmund Walker (banker)
#   341 -> Andrés D'Alessandro (footballer)
#   343 -> Keith Park / Arthur Noss
#   357 -> Dirkie Chamberlain (Olympic athlete)
#
#   --- New, from 361-440 ---
#   360 -> Irma Pamela Hall (actress)
#   361 -> Marvin Heemeyer / Andrey Zvyagintsev
#   362 -> Alan Furst / Lee Child (authors)
#   363 -> Bruce Springsteen
#   367 -> Tuppence Middleton (actress)
#   370 -> Baker Mayfield / Trevor Knight (NFL players)
#   372 -> V. K. Krishna Menon / Jawaharlal Nehru
#   375 -> Matthew Kelly / Peter Davison (actors/presenters)
#   377 -> Oliver Reed (actor)
#   379 -> John the Blind (King of Bohemia)
#   380 -> Simon Wincer / Patrice Leconte (directors)
#   381 -> Ottokar Kernstock / Joseph Haydn
#   382 -> Lincoln Barrett "High Contrast" (DJ)
#   383 -> Nobuo Uematsu (composer)
#   388 -> Carl Boese / Franco Zeffirelli (directors)
#   390 -> Donald Trump / Hillary Clinton
#   395 -> Patrick Carpentier (racing driver)
#   397 -> Ian Hunter / Stuart Murdoch (musicians)
#   399 -> Garrett Smithley (racing driver)
#   400 -> Bart Conner / Nadia Comăneci (Olympic gymnasts)
#   401 -> Mike Williams / Tommy Morrison (actor/boxer)
#   402 -> Richard Van Patten (actor)
#   403 -> Richard Bedford / Armin van Buuren (musicians)
#   405 -> Chris Farley (comedian)
#   408 -> Vincent Walker / David Usher (musicians)
#   409 -> Yukio Mishima / Roberto Bolaño (authors)
#   411 -> Andrew Bird (musician)
#   412 -> Stacey Kent / Jim Tomlinson (jazz musicians)
#   413 -> Jack Sock / Raffaella Reggi (tennis players)
#   414 -> Dolly Parton
#   416 -> Paul "Bear" Bryant / Mark Ingram Jr.
#   417 -> Laura Ellen Kirk (actress)
#   422 -> Willie Nelson (The Highwaymen)
#   423 -> Charles Hutchison / Reginald Barker (directors)
#   429 -> Randal Kleiser / Kyle Schickner (directors)
#   430 -> Barbara Jordan / Mark Knowles (tennis players)
#   431 -> Lyndon B. Johnson (via Thomas C. Mann entry)
#   432 -> Elyes Gabel (actor)
#   434 -> Sia / Garth Davis (musician/director)
#   435 -> Lee Hong-gi / Dee Snider (musicians)
#   438 -> Billy Corgan (Smashing Pumpkins)
#   439 -> Conan O'Brien / Brandon Tartikoff
BATCH_INDICES = [
    290, 316, 332, 333, 337, 341, 343, 357, 360, 361, 362, 363,
    367, 370, 372, 375, 377, 379, 380, 381, 382, 383, 388, 390,
    395, 397, 399, 400, 401, 402, 403, 405, 408, 409, 411, 412,
    413, 414, 416, 417, 422, 423, 429, 430, 431, 432, 434, 435,
    438, 439
]

BASE_OUTPUT_DIR = "outputs_strict_batch_361_440_combined"
...
print(f"\n{'='*65}")
print(f"PIPELINE_STRICT_BATCH_361_440_COMBINED COMPLETE — {total_time:.1f}s this run")
print(f"{'='*65}")
for r in results:
    print(
...
report_lines = [
    "="*65,
    "PIPELINE_STRICT_BATCH_361_440_COMBINED — FINAL REPORT",
    "="*65,
