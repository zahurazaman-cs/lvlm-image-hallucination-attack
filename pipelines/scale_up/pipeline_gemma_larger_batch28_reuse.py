# ============================================================
# pipeline_gemma_larger_batch28_reuse.py
# ============================================================
# Scale-up experiment: tests whether a LARGER parameter version of
# Gemma-4-abliterated, as the victim model, is more or less
# susceptible to the same attack than the smaller version used
...
# regenerated.
#
#   Step 8 (THE ONLY VARIABLE) — the LARGER Gemma-4-abliterated
#            model answers, using the same two images, the same
#            unmodified original attack prompt used everywhere
#            else in this project.
#   Step 9 (UNCHANGED) — the ORIGINAL, SMALLER Gemma-4-abliterated
#            model still does the judging, kept fixed exactly as
#            in every other victim-swap experiment in this
#            project, so only the victim model differs, not the
#            judge.
#
# IMPORTANT: set LARGER_VICTIM_MODEL below to the correct Ollama
# tag before running. Run `ollama list | grep gemma-4-abliterated`
# first to see which size you currently have, then pick a larger
# one from: e2b, e4b, 12b, 26b, 31b, 48b.
# ============================================================
...
JUDGE_MODEL  = "huihui_ai/gemma-4-abliterated"             # UNCHANGED — your original model, exactly as used throughout this whole project
LARGER_VICTIM_MODEL = "huihui_ai/gemma-4-abliterated:12b"  # genuinely larger — 12B parameters vs your current 8.0B

OLLAMA_URL = "http://localhost:11434/api"
OUTPUT_DIR = "outputs_gemma_larger_batch28_reuse"
...
def ollama_text(prompt, model, system=""):
    payload = {
        ...
        "think": False,  # disable reasoning trace — go straight to the answer
...
# (entry_num, final_category, final_iteration) — right_answer and
# hallucinated_answer live in the ORIGINAL SUMMARY.json, so both
# files are loaded and merged by entry_num ────────────────────────
...
print(f"\n{'='*65}")
print(f"BATCH 28 SCALE-UP TEST")
print(f"Victim (LARGER): {LARGER_VICTIM_MODEL}")
print(f"Judge (unchanged, original size): {JUDGE_MODEL}")
...
for e in all_entries:
    ...
    print(f"\n{key}  (original smaller-model status: {e['original_status']})")
...
Do NOT say "not stated" or "unknown":"""
        ...
        with open(f"{out_entry_dir}/step8_answer.txt", "w") as f:
            f.write(
                ...
                f"Original Smaller-Model Status: {e['original_status']}\n"
            )
...
REASON: [brief explanation]"""
        ...
        print(f"  Judge (original smaller model): {hall_result[:120]}")
    ...
    print(f"  Larger model classification: {larger_status}")
    ...
    state[key] = {
        ...
        "victim_model":         LARGER_VICTIM_MODEL,
    }
    save_state(state)
...
print(f"\n{'='*65}")
print(f"COMPLETE — {LARGER_VICTIM_MODEL} on batch 28's 28 entries")
print(f"{'='*65}")
...
# ── Original smaller model result, for direct comparison ────────
orig_t = sum(1 for e in all_entries if e["original_status"] == "TARGETED")
orig_u = sum(1 for e in all_entries if e["original_status"] == "UNTARGETED")
orig_n = sum(1 for e in all_entries if e["original_status"] == "NONE")
orig_total = orig_t + orig_u + orig_n
print(f"\nFor comparison, the ORIGINAL SMALLER model's result on these "
      f"same 28 entries: {orig_t+orig_u}/{orig_total} "
