# ============================================================
# piechart_qwen25_nqswap_100.py
# ============================================================
# Pie chart for the Qwen2.5-VL victim results on all 100 NQ-Swap
# entries, using the ORIGINAL, unmodified attack prompt (no
# defense wrapping applied yet).
#
# Reads: outputs_victim_qwen_nqswap_100/entry_state.json
#
# Victim: Qwen2.5-VL 7B  |  Judge: Gemma-4-abliterated (unchanged)
# Dataset: NQ-Swap (Longpre et al.) — independent of HaluEval/
# HotpotQA, same 100 entries already tested against Gemma as
# victim.
#
# Saves the chart to:
#   outputs_victim_qwen_nqswap_100/piechart_qwen25_nqswap_100.png
#
...
CHART_PATH = "outputs_victim_qwen_nqswap_100/piechart_qwen25_nqswap_100.png"
...
if scored_total == 0:
    ...
    print(f"TOTAL hallucination      : {targeted + untargeted} "
          f"({total_hall_pct:.1f}%)")
    print()
    print(f"For comparison, Gemma as victim on these same 100 "
          f"NQ-Swap entries gave 48.0% total hallucination.")
    ...
    ax.set_title(
        f"Hallucination Classification — NQ-Swap, Qwen2.5-VL Victim "
        f"({scored_total} Entries)\n"
        f"Victim: Qwen2.5-VL 7B  |  Judge: Gemma-4-abliterated  |  "
        f"Original Attack Prompt (no defense)\n"
        f"Same 100 entries as the Gemma-victim NQ-Swap test, images "
        f"reused, only Step 8 model changed\n"
