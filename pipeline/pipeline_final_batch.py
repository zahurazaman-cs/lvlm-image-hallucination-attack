# ============================================================
# pipeline_final_batch.py — Batch Production Pipeline
# ============================================================
# Runs the same proven pipeline as pipeline_final.py across all
# human-celebrity entries identified within the first 100 lines
# of qa_data.json.
#
# KEY DESIGN (identical core logic to pipeline_final.py):
# - Step 4: Multiple targeted SERP queries per entry to get
#           the RIGHT person's photo (or couple/comparison photo)
...
#
# NEW IN BATCH VERSION:
# - RESUMABLE: skips entries already completed in a prior run by
#   checking outputs_final_batch/SUMMARY.json at startup
# - GENERIC SERP QUERIES: auto-derives an occupation/context hint
#   per entry instead of relying on a hand-written per-entry dict
#   (impractical for 28 different people)
# - MAX_STEP10_ITER = 5 (per instruction: max 5 iterations per
#   entry, then move on to the next entry regardless of result)
# ============================================================
...
# 0-indexed line numbers in qa_data.json for the 28 human-celebrity
# entries identified within the first 100 lines.
#
#   line#  ->  subject
#    2  -> Allie Goertz / Richard Nixon
#    3  -> Peggy Seeger
#    5  -> Jonathan Stark
#    7  -> Badr Hari
#   15  -> Annette Bening
...
#   36  -> David Lee Roth
#   44  -> Tammy Wynette
#   50  -> Glenn Hughes / Ross Lynch
#   53  -> Cynthia Nixon
#   54  -> Bruce Lee
#   58  -> Ginger Rogers
#   63  -> Victor Mature
#   65  -> Anthony Eden
#   67  -> Tim McIlrath / Spike Slawson
#   68  -> John Goodman / Zooey Deschanel
#   69  -> Tom Daley
#   70  -> Keyshia Cole
#   76  -> Christopher Hitchens / Richard Dawkins
#   77  -> Dwight Eisenhower
#   79  -> Garth Brooks
#   83  -> Kim Yool-ho
#   85  -> Max Gail
#   93  -> Swoosie Kurtz
#   94  -> Asa Griggs Candler
BATCH_INDICES = [
    2, 3, 5, 7, 15, 18, 22, 26, 30, 36, 44, 50, 53, 54, 58, 63, 65,
    67, 68, 69, 70, 76, 77, 79, 83, 85, 93, 94
]

BASE_OUTPUT_DIR = "outputs_final_batch"
...
def fetch_photo(person_name, context_hint, output_dir):
    ...
    player", "actress", "kickboxer") extracted per-entry by the
    LLM, used to disambiguate common names and improve hit quality.
    """
    queries = [
...
def create_newspaper_png(image_path, headline, article_text,
    ...
    # Caption: "Figure 1 - [short clean caption]" (no truncation needed
    # since caption_text is already a short fixed-format string)
...
def run_entry(entry_num, entry):
    ...
    # ── STEP 3.5: Extract occupation/context hint for SERP ────
    ...
    print(f"STEP 3.5 — Context hint: {context_hint}")
...
Output the SAME passage with [Figure 1] inserted exactly once:"""
        ...
        # Short, clean caption — fixed format, never truncated
...
# ── Resumability: load any prior results and skip completed entries ──
results = []
already_done = set()
...
print(f"\n{'='*65}")
print(f"PIPELINE_FINAL_BATCH COMPLETE — {total_time:.1f}s this run")
print(f"{'='*65}")
for r in results:
    print(
...
report_lines = [
    "="*65,
    "PIPELINE_FINAL_BATCH — FINAL REPORT",
    "="*65,
    ...
    "  Step 4: Generic per-entry SERP queries (person + auto context hint)",
    ...
    "          short fixed-format caption (no truncation)",
