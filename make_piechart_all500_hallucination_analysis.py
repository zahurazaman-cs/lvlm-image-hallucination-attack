# ============================================================
# make_piechart_all500_hallucination_analysis.py
# ============================================================
# Pie chart for the all-500 Structured Hallucination-Analysis Defense test
# (outputs_step8_reclassify_all500_hallucination_analysis) — tests
# what the Gemma-victim attack's overall hallucination rate would
# have been if the professor-approved HALLUCINATION_ANALYSIS
# prompt had been used from the very start, using the EXACT SAME
# existing final images for all 500 entries (Steps 1-7 untouched).
#
...
# hallucination) and to the earlier Minimal Prompt Defense result
# (281/500, 56.2% total hallucination).
#
# Saves the chart to:
#   outputs_step8_reclassify_all500_hallucination_analysis/piechart_all500_hallucination_analysis.png
#
...
STATE_PATH = "outputs_step8_reclassify_all500_hallucination_analysis/entry_state.json"
CHART_PATH = ("outputs_step8_reclassify_all500_hallucination_analysis/"
              "piechart_all500_hallucination_analysis.png")
...
if scored_total == 0:
    ...
    print()
    print(f"For comparison across all 500 entries:")
    print(f"  No Defense                    : 66.4% total hallucination")
    print(f"  Minimal Prompt Defense         : 56.2% total hallucination")
    print(f"  Structured Hallucination-Analysis Defense: {total_hall_pct:.1f}% "
          f"total hallucination")
    ...
    ax.set_title(
        ...
        f"Model: Gemma-4-abliterated\n"
        f"Same 500 entries, same existing images (Steps 1-7 "
        f"untouched)\n"
        f"Total Hallucination Rate: {total_hall_pct:.1f}% "
        f"(Targeted + Untargeted)  |  vs. No Defense: 66.4%  |  "
        f"vs. Minimal Prompt Defense: 56.2%",
